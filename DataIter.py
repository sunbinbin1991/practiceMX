...
class MsuRecordIter(mx.io.DataIter):
    ...
    ----------
    """
    def __init__(self, path_imgrec, batch_size, data_shape = (3,96,96), path_imglist="",
    ...
    def _get_batch(self):
        ...
        for i in range(self.batch_size):
            ...
            # get origin image and obtain its data in different channel
...
class LoadIter(mx.io.DataIter):
    ...
    def __init__(self, imdb, batch_size, data_shape, \
        ...
        self._getDict()
        self._size =    self._data_dict.__len__()# get count of number
    ...
    def _getDict(self):
        if self.is_train:
            path_list = self._imdb.train_List
        else:
            path_list = self._imdb.val_List
        with open(path_list) as f:
            ...
            while True:
                ...
                label = line.split("\t")[1]
                path = line.split("\t")[2]
                data_dict[int(index)] = path
                label_dict[int(index)] = label
        self._data_dict =  data_dict
        self._label_dict = label_dict
        return self._data_dict,self._label_dict
    ...
    def _get_batch(self):
        ...
        for i in range(self.batch_size):
            ...
            im_path = self._data_dict[index]
            im_path = im_path.replace("\r\n", "").replace("\\","/")
            with open(im_path, 'rb') as fp:
                img_content = fp.read()
            img = mx.img.imdecode(img_content)
            gt = self._data_dict[index].copy() if self.is_train else None
