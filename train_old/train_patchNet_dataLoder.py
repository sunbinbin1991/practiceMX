...
from DataIter import MsuRecordIter,LoadIter
...
def load_iterators(batch_size):
    path = "D:/face_lib\FASD/train/"
    batch_size = 64
    data_shape = (3,96,96)
    train_list = "train_train.lst"
    test_list = "train_val.lst"

    Imdb.num_images = 3222
    Imdb.train_List = train_list
    Imdb.val_list = train_list
    train_iter = LoadIter(Imdb,batch_size, data_shape, mean_pixels=[128, 128, 128], rand_samplers=[],
                 rand_mirror=False, shuffle=False, rand_seed=None, is_train=True, max_crop_trial=50)
...
if __name__ == "__main__":
    ...
    (train, val) = load_iterators(batch_size)
